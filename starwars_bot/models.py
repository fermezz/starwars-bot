...
Known Residents: {", ".join(self.residents_names) if self.residents_names else "None"}
"""
...
class Person:
    ...
    def __init__(self, **kwargs):
        ...
        self.starships = kwargs["starships"]
        # films = kwargs["films"]
...
Mass: {make_title_if_unknown(self.mass)}{"kg" if self.mass != "unknown" else ""}
...
Homeworld: {make_title_if_unknown(self.homeworld_name.title())}
Known Starships: {", ".join(self.starships_names) if self.starships_names else "None"}
"""
    ...
    @cached_property
    def starships_names(self):
        return [requests.get(url=starship).json()["name"] for starship in self.starships]
